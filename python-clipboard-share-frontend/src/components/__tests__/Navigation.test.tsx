...
describe("Navigation", () => {
  ...
  it("renders the Python Clipboard Share logo", () => {
    renderWithAuth(<Navigation />);
    expect(screen.getByText("Python Clipboard Share")).toBeInTheDocument();
  });

  it("renders all navigation items", () => {
    renderWithAuth(<Navigation />);
    // Check that navigation items exist (they appear in both desktop and mobile)
    expect(screen.getAllByText("Clipboard")).toHaveLength(2);
  });

  it("renders navigation links with correct hrefs", () => {
    renderWithAuth(<Navigation />);
    // Get desktop navigation links - use exact match to exclude logo
    const clipboardLinks = screen.getAllByRole("link", {
      name: "Clipboard",
    });
    expect(clipboardLinks[0]).toHaveAttribute("href", "/clipboard");
  });

  it("applies active styling to current page", () => {
    mockUsePathname.mockReturnValue("/clipboard");
    ...
    expect(clipboardLinks[0]).toHaveClass("font-bold", "text-border-accent");
    expect(clipboardLinks[1]).toHaveClass("font-bold", "text-border-accent");
  });
  ...
  describe("Mobile Navigation", () => {
    ...
    it("closes mobile menu when navigation link is clicked", () => {
      ...
      const clipboardLink = clipboardLinks[1]; // Second one is in mobile menu
      if (clipboardLink) {
        fireEvent.click(clipboardLink);
      }
    ...
    it("applies active styling to current page in mobile menu", () => {
      ...
      const mobileClipboardLink = clipboardLinks[1]; // Second one is in mobile menu
      expect(mobileClipboardLink).toHaveClass(
        "font-bold",
        "text-border-accent",
      );
    });
  ...
  describe("HealthIndicator integration", () => {
    ...
    it("positions HealthIndicator next to the logo", () => {
      ...
      expect(nav).toContainElement(screen.getByText("Python Clipboard Share"));
